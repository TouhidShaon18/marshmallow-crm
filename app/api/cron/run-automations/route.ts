...
import { syncWooCommerce } from "@/lib/woo";
...
export async function GET(req: Request): Promise<Response> {
  ...
  try {
    ...
    // 5. WooCommerce affiliate sync — pull completed orders that used a creator coupon
    let wooSynced = 0;
    if ((await getSetting("woo_sync_enabled")) === "true") {
      const woo = await syncWooCommerce();
      if (woo.ok) wooSynced = woo.created ?? 0;
      else console.warn("Woo sync skipped:", woo.message);
    }
